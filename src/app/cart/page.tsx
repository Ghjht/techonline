  ...
  if (items.length === 0) {
    return (
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-20 text-center">
        ...
        <p className="text-gray-500 mb-8">Explorez notre catalogue et ajoutez des produits.</p>
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
      <div className="flex items-center justify-between mb-8">
        <h1 className="text-2xl font-bold text-gray-900">Votre Panier ({totalItems} articles)</h1>
        <button
          onClick={clearCart}
      ...
      <div className="space-y-4 mb-8">
        {items.map((item) => (
          <div key={item.product.id} className="bg-white border border-gray-200 rounded-xl p-4 flex gap-4">
            <Link href={`/products/${item.product.slug}`} className="w-20 h-20 sm:w-24 sm:h-24 bg-gray-100 rounded-lg overflow-hidden flex-shrink-0">
              <img src={item.product.image} alt={item.product.name} className="w-full h-full object-cover" />
            </Link>

            <div className="flex-1 min-w-0">
              <Link href={`/products/${item.product.slug}`}>
                <h3 className="font-semibold text-gray-900 text-sm sm:text-base hover:text-primary-600 transition-colors line-clamp-2">
                  {item.product.name}
                </h3>
              </Link>
              <p className="text-xs text-gray-500 mt-0.5">{item.product.brand}</p>
              <p className="font-bold text-gray-900 mt-1">{formatPrice(item.product.price * item.quantity)}</p>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-6">
        ...
        <div className="flex justify-between items-center mb-4 text-sm text-gray-500">
          <span>Livraison</span>
          <span>Offerte (France métropolitaine)</span>
        </div>
