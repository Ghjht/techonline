...
export default function ProductDetailPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
      ...
      <div className="mt-12">
        <h2 className="text-xl font-bold text-gray-900 mb-6">Spécifications techniques</h2>
        <div className="border border-gray-200 rounded-xl overflow-hidden">
          {Object.entries(product.specs).map(([key, value], idx) => (
            <div key={key} className={`flex px-6 py-3 ${idx % 2 === 0 ? "bg-gray-50" : "bg-white"}`}>
              <span className="w-1/2 text-sm font-medium text-gray-900">{key}</span>
              <span className="w-1/2 text-sm text-gray-600">{value}</span>
            </div>
          ))}
        </div>
      </div>
